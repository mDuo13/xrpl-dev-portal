...
The `nft_history` command asks the Clio server for past transaction metadata for the [NFT](../../../../concepts/tokens/non-fungible-tokens.md) being queried. [New in: Clio v1.1.0](https://github.com/XRPLF/clio/releases/tag/1.1.0 "BADGE_BLUE")
...
For definitions of the fields returned in the `tx` object, see [Transaction Metadata](../../../protocol-reference/transactions/transaction-metadata.md).
...
* `lgrIdxsInvalid` - Either the request specifies a `ledger_index_max` that is before the `ledger_index_min`, or the server does not have a validated ledger range because it is [not synced with the network](../../../../infrastructure/rippled/troubleshooting/server-doesnt-sync.md).
