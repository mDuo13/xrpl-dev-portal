...
The `sfMasterSignature` and `sfSignature` signatures are created from signing the [serialized](../../protocol-reference/serialization.md) binary data of the manifest, excluding the signature fields (`sfMasterSignature` and `sfSignature`) themselves.
