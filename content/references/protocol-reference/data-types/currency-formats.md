...
The XRP Ledger has two kinds of digital asset: XRP and [tokens](../../../concepts/tokens/tokens.md). Both types have high precision, although their formats are different.
...
| Can't be [frozen](../../../concepts/tokens/freezes.md).                         | The issuer can [freeze](../../../concepts/tokens/freezes.md) balances. |
| No transfer fees; XRP-to-XRP payments are always direct. | Can take indirect [paths](../../../concepts/tokens/paths.md) with each issuer charging a percentage [transfer fee](../../../concepts/tokens/transfer-fees.md). |
...
For more information, see [What is XRP?](../../../concepts/introduction/what-is-xrp.md) and [Tokens](../../../concepts/tokens/tokens.md).
...
To specify an amount of a [(fungible) token](../../../concepts/tokens/tokens.md), use an Amount object. This is a JSON object with three fields:
...
| `issuer`   | String                     | Generally, the [account](../../../concepts/accounts/accounts.md) that issues this token. In special cases, this can refer to the account that holds the token instead (for example, in a [Clawback](../transactions/transaction-types/clawback.md) transaction). |
...
In some cases, you need to define an asset (which could be XRP or a token) without a specific amount, such as when defining an order book in the [decentralized exchange](../../../concepts/tokens/decentralized-exchange.md).
...
When sending token amounts in the XRP Ledger's peer-to-peer network, servers [serialize](../serialization.md) the amount to a 64-bit binary value.

**Tip:** For tokens that should not be divisible at all, see [Non-Fungible Tokens (NFTs)](../../../concepts/tokens/non-fungible-tokens.md).
...
At the protocol level, this format is [serialized](../serialization.md#currency-codes) into a 160-bit binary value starting with `0x00`.
