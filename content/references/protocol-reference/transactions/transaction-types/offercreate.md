...
An OfferCreate transaction places an [Offer](../../../../concepts/tokens/offers.md) in the [decentralized exchange](../../../../concepts/tokens/decentralized-exchange.md).
...
|:---------------|:--------------------|:------------------|:------------------|
| [`Expiration`](../../../../concepts/tokens/offers.md#offer-expiration) | Number | UInt32 | _(Optional)_ Time after which the Offer is no longer active, in [seconds since the Ripple Epoch][]. |
...
Transactions of the OfferCreate type support additional values in the [`Flags` field](../transaction-common-fields.md#flags-field), as follows:
...
| `tecFROZEN`              | Occurs if the transaction involves a token on a [frozen](../../../../concepts/tokens/freezes.md) trust line (including local and global freezes). |
...
| `tecNO_LINE`             | Occurs if the transaction involves a token whose issuer uses [Authorized Trust Lines](../../../../concepts/tokens/authorized-trust-lines.md) and the necessary trust line does not exist. |
| `tecNO_AUTH`             | Occurs if the transaction involves a token whose issuer uses [Authorized Trust Lines](../../../../concepts/tokens/authorized-trust-lines.md) and the the trust line that would receive the tokens exists but has not been authorized. |
