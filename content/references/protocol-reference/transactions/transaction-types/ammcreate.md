...
Create a new [Automated Market Maker](../../../../concepts/tokens/automated-market-makers.md) (AMM) instance for trading a pair of assets ([fungible tokens](../../../../concepts/tokens/tokens.md) or [XRP](xrp.html)).
...
One or both of `Amount` and `Amount2` can be [tokens](../../../../concepts/tokens/tokens.md); at most one of them can be [XRP](xrp.html). They cannot both have the same currency code and issuer. The tokens' issuers must have [Default Ripple](../../../../concepts/tokens/rippling.md#the-default-ripple-flag) enabled. If the [Clawback amendment][] :not_enabled: is enabled, those issuers must not have enabled the Allow Clawback flag. An AMM's LP tokens _can_ be used as one of the assets for another AMM.
...
| `tecFROZEN`         | At least one of the deposit assets (`Amount` or `Amount2`) is currently [frozen](../../../../concepts/tokens/freezes.md). |
...
| `tecNO_AUTH`        | At least one of the deposit assets uses [authorized trust lines](../../../../concepts/tokens/authorized-trust-lines.md) and the sender does not have authorization to hold that asset. |
...
| `terNO_RIPPLE`      | The issuer of at least one of the assets has not enabled the [Default Ripple flag](../../../../concepts/tokens/rippling.md#the-default-ripple-flag). |
