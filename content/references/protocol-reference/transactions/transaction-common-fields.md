...
Every transaction has the same set of common fields, plus additional fields based on the [transaction type](transaction-types/transaction-types.md). Field names are case-sensitive. The common fields for all transactions are:
...
| `TransactionType`    | String           | UInt16            | _(Required)_ The type of transaction. Valid [transaction types](transaction-types/transaction-types.md) include: `Payment`, `OfferCreate`, `TrustSet`, and many others. |
...
| [`Signers`](#signers-field) | Array     | Array             | _(Optional)_ Array of objects that represent a [multi-signature](../../../concepts/transactions/multi-signing.md) which authorizes this transaction. |
...
| `TicketSequence`   | Number           | UInt32            | _(Optional)_ The sequence number of the [ticket](../../../concepts/transactions/tickets.md) to use in place of a `Sequence` number. If this is provided, `Sequence` must be `0`. Cannot be used with `AccountTxnID`. |
...
[Removed in: rippled 0.28.0](https://github.com/XRPLF/rippled/releases/tag/0.28.0 "BADGE_RED"): The `PreviousTxnID` field of transactions was replaced by the [`AccountTxnID`](#accounttxnid) field. This String / Hash256 field is present in some historical transactions. This is unrelated to the field also named `PreviousTxnID` in some [ledger objects](../ledger-data/ledger-data-formats.md).
...
The `AccountTxnID` field cannot be used on transactions that use [Tickets](../../../concepts/transactions/tickets.md). Transactions that use `AccountTxnID` cannot be placed in the [transaction queue](../../../concepts/transactions/transaction-queue.md).
...
The [`Paths` field](transaction-types/payment.md#paths) of the [Payment transaction](transaction-types/payment.md) type can also be automatically filled in.
...
| Type-based Flags | `0x00ff0000` | Flags with different meanings depending on the [transaction type](transaction-types/transaction-types.md) that uses them. |
| Reserved Flags   | `0x0000ffff` | Flags that are not currently defined. A transaction is only valid if these flags are disabled. |

**Note:** The [AccountSet transaction](transaction-types/accountset.md) type has [its own non-bitwise flags](transaction-types/accountset.md#accountset-flags), which serve a similar purpose to type-based flags. [Ledger objects](ledger-object-types.html) also have a `Flags` field with different bitwise flag definitions.
...
| Field        | Type   | [Internal Type](../serialization.md) | Description                      |
|:-------------|:-------|:------------------|:---------------------------------|
...
- The sender's `Sequence` number on the second network matches the transaction's `Sequence`, or the transaction uses a [Ticket](../../../concepts/transactions/tickets.md) that's available on the second network.
...
The `Signers` field contains a [multi-signature](../../../concepts/transactions/multi-signing.md), which has signatures from up to 32 key pairs, that together should authorize the transaction. The `Signers` list is an array of objects, each with one field, `Signer`. The `Signer` field has the following nested fields:
