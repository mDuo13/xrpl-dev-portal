...
| `AccountTxnID`                | String    | Hash256           | No        | The identifying hash of the transaction most recently sent by this account. This field must be enabled to use the [`AccountTxnID` transaction field](../../transactions/transaction-common-fields.md#accounttxnid). To enable it, send an [AccountSet transaction with the `asfAccountTxnID` flag enabled](../../transactions/transaction-types/accountset.md#accountset-flags). |
...
| `BurnedNFTokens`              | Number    | UInt32            | No        | How many total of this account's issued [non-fungible tokens](../../../../concepts/tokens/non-fungible-tokens.md) have been burned. This number is always equal or less than `MintedNFTokens`. |
...
| `FirstNFTokenSequence`        | Number    | UInt32            | No        | The account's [Sequence Number][] at the time it minted its first [non-fungible-token](../../../../concepts/tokens/non-fungible-tokens.md). _(Added by the [fixNFTokenRemint amendment][] :not_enabled:)_ |
...
| `MintedNFTokens`              | Number    | UInt32            | No        | How many total [non-fungible tokens](../../../../concepts/tokens/non-fungible-tokens.md) have been minted by and on behalf of this account. _(Added by the [NonFungibleTokensV1_1 amendment](known-amendments.html#nonfungibletokensv1_1))_ |
| `NFTokenMinter`               | String    | AccountID         | No        | Another account that can mint [non-fungible tokens](../../../../concepts/tokens/non-fungible-tokens.md) on behalf of this account. _(Added by the [NonFungibleTokensV1_1 amendment](known-amendments.html#nonfungibletokensv1_1))_ |
...
| `RegularKey`                  | String    | AccountID         | No        | The address of a [key pair](../../../../concepts/accounts/cryptographic-keys.md) that can be used to sign transactions for this account instead of the master key. Use a [SetRegularKey transaction](../../transactions/transaction-types/setregularkey.md) to change this value. |
...
- `lsfDepositAuth` **enabled** and no accounts preauthorized. This ensures that the only way to add money to the AMM Account is using the [AMMDeposit transaction](../../transactions/transaction-types/ammdeposit.md).
...
Many AccountRoot flags correspond to options you can change with an [AccountSet transaction](../../transactions/transaction-types/accountset.md). However, the bit values used in the ledger are different than the values used to enable or disable those flags in a transaction. Ledger flags have names that begin with **`lsf`**.

AccountRoot objects can have the following flags combined in the `Flags` field:

| Flag Name                         | Hex Value    | Decimal Value | Corresponding [AccountSet Flag](../../transactions/transaction-types/accountset.md#accountset-flags) | Description |
|-----------------------------------|--------------|-------------------|-----------------------------------|----|
| `lsfAllowTrustLineClawback`       | `0x80000000` | 2147483648        | `asfAllowTrustLineClawback`       | Enable [Clawback](../../../../concepts/tokens/clawing-back-tokens.md) for this account. _(Requires the [Clawback amendment][].)_ |
| `lsfDefaultRipple`                | `0x00800000` | 8388608           | `asfDefaultRipple`                | Enable [rippling](../../../../concepts/tokens/rippling.md) on this addresses's trust lines by default. Required for issuing addresses; discouraged for others. |
...
The [reserve](../../../../concepts/accounts/reserves.md) for an AccountRoot entry is the base reserve, currently {{target.base_reserve}}, except in the case of a special AMM AccountRoot.
