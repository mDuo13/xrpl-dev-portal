...
A Ticket is a record that a sequence number has been set aside to be used later. An account first sends a [TicketCreate transaction](../../references/protocol-reference/transactions/transaction-types/ticketcreate.md) to set aside one or more sequence numbers as Tickets; this puts a record in the [ledger's state data](../ledgers/ledgers.md), in the form of a [Ticket object](../../references/protocol-reference/ledger-data/ledger-entry-types/ticket.md), for each sequence number reserved.
...
As with sequence numbers, sending a transaction uses up the Ticket _if and only if_ the transaction is confirmed by [consensus](../consensus-protocol/consensus.md). However, transactions that fail to do what they were intended to do can still be confirmed by consensus with [`tec`-class result codes](../../references/protocol-reference/transactions/transaction-results/tec-codes.md).
...
- To "cancel" a Ticket, use the Ticket to [perform a no-op](about-canceling-a-transaction.md) [AccountSet transaction](../../references/protocol-reference/transactions/transaction-types/accountset.md). This deletes the Ticket so that you don't have to meet its reserve requirement.
...
- **References:**
    ...
    - [Ticket object](../../references/protocol-reference/ledger-data/ledger-entry-types/ticket.md)
