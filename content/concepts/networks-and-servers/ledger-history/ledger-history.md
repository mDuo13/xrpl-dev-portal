...
Backfilling history is one of the server's lowest priorities, so it may take a long time to fill missing history, especially if the server is busy or its hardware and network specs aren't good enough. For recommendations on hardware specs, see [Capacity Planning](../../../infrastructure/rippled/installation/capacity-planning.md). Backfilling history also requires that at least one of the server's direct peers has the history in question. For more information on managing your server's peer-to-peer connections, see [Configure Peering](../../../infrastructure/rippled/configure-peering/configure-peering.md).

The XRP Ledger identifies data (on several different levels) by a unique hash of its contents. The XRP Ledger's state data contains a short summary of the ledger's history, in the form of the [LedgerHashes object type](../../../references/protocol-reference/ledger-data/ledger-entry-types/ledgerhashes.md). Servers use the LedgerHashes objects to know which ledger versions to fetch, and to confirm that the ledger data they receive is correct and complete.
...
For instructions on setting up full history, see [Configure Full History](../../../infrastructure/rippled/configuration/configure-full-history.md).

## History Sharding

An alternative to storing the full history of the XRP Ledger on a single expensive machine is to configure many servers to each store a part of all ledger history. The [History Sharding](history-sharding.md) feature makes this possible, storing ranges of ledger history in a separate storage area called the _shard store_. When a peer server asks for specific data (as described in [fetching history](#fetching-history) above), a server can answer using data from either its ledger store or shard store.
...
For more information, see [Configure History Sharding](../../../infrastructure/rippled/configuration/configure-history-sharding.md).


## See Also

- **Concepts:**
    - [Ledgers](../../ledgers/ledgers.md)
    - [Consensus](../../consensus-protocol/consensus.md)
- **Tutorials:**
    - [Configure `rippled`](configure-rippled.html)
        - [Configure Online Deletion](../../../infrastructure/rippled/configuration/configure-online-deletion.md)
        - [Configure Advisory Deletion](../../../infrastructure/rippled/configuration/configure-advisory-deletion.md)
        - [Configure History Sharding](../../../infrastructure/rippled/configuration/configure-history-sharding.md)
        - [Configure Full History](../../../infrastructure/rippled/configuration/configure-full-history.md)
- **References:**
