...
Ideally, the server should be able to send _and_ receive connections on the peer port. You should [forward the port used for the peer protocol through your firewall](../../infrastructure/rippled/configure-peering/forward-ports-for-peering.md) to the `rippled` server.
...
The node key pair also identifies other servers for purposes of [clustering](clustering.md) or [reserving peer slots](#fixed-peers-and-peer-reservations). If you have a cluster of servers, you should configure each server in the cluster with a unique `[node_seed]` setting. For more information on setting up a cluster, see [Cluster `rippled` Servers](../../infrastructure/rippled/configure-peering/cluster-rippled-servers.md).
...
To configure your server as a private server, set the `[peer_private]` setting to `1` in the config file. For detailed instructions, see [Configure a Private Server](../../infrastructure/rippled/configure-peering/configure-a-private-server.md).
...
- **Tutorials:**
    - [Cluster rippled Servers](../../infrastructure/rippled/configure-peering/cluster-rippled-servers.md)
    - [Configure a Private Server](../../infrastructure/rippled/configure-peering/configure-a-private-server.md)
    - [Configure the Peer Crawler](../../infrastructure/rippled/configure-peering/configure-the-peer-crawler.md)
    - [Forward Ports for Peering](../../infrastructure/rippled/configure-peering/forward-ports-for-peering.md)
    - [Manually Connect to a Specific Peer](../../infrastructure/rippled/configure-peering/manually-connect-to-a-specific-peer.md)
    - [Set Maximum Number of Peers](../../infrastructure/rippled/configure-peering/set-max-number-of-peers.md)
    - [Use a Peer Reservation](../../infrastructure/rippled/configure-peering/use-a-peer-reservation.md)
- **References:**
