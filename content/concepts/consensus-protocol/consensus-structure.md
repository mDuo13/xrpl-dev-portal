...
This article provides a high level overview of the XRP Ledger, the information it stores, and how [transactions](../../references/protocol-reference/transactions/transaction-formats.md) result in changes to the ledger.
...
- balances of XRP and [tokens](../tokens/tokens.md)
...
For a full, technical description of which data is included in a ledger version, see the [Ledger Format Reference](../../references/protocol-reference/ledger-data/ledger-data-formats.md).
...
User level changes to the ledger are the results of transactions. Examples of [transactions](../../references/protocol-reference/transactions/transaction-formats.md) include payments, changes to account settings or trust lines, and offers to trade. Each transaction authorizes one or more changes to the ledger, and is cryptographically signed by an account owner. Transactions are the only way to authorize changes to an account, or to change anything else in the ledger.
...
3. Process each transaction according to its instructions, in order. Update the ledger's state data accordingly.
    ...
    If the transaction cannot be successfully executed, include the transaction with a [`tec`-class result code](../../references/protocol-reference/transactions/transaction-results/tec-codes.md).<a href="#footnote_1" id="from_footnote_1"><sup>1</sup></a>
