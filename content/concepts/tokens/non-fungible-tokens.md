...
The XRP Ledger offers support for [tokens](tokens.md). Such assets are, primarily, fungible.
...
On the XRP Ledger, a non-fungible token is represented as a [NFToken](../../references/protocol-reference/data-types/nftoken.md) object. A `NFToken` is a unique, indivisible unit that is not used for payments. Users can mint (create), hold, buy, sell, and burn (destroy) such tokens.

The ledger stores up to 32 `NFToken` objects owned by the same account in a single [NFTokenPage object](../../references/protocol-reference/ledger-data/ledger-entry-types/nftokenpage.md) to save space. As a result, the owner's [reserve requirement](../accounts/reserves.md) for `NFToken` objects only increases when the ledger needs to make a new page to store additional tokens.
...
- Whether the holder can sell the `NFToken` for [fungible token](tokens.md) amounts, or only for XRP.


## `NFToken` Lifecycle

Anyone can create a new `NFToken` using the [NFTokenMint transaction](../../references/protocol-reference/transactions/transaction-types/nftokenmint.md) type. The `NFToken` lives on the [NFTokenPage object](../../references/protocol-reference/ledger-data/ledger-entry-types/nftokenpage.md) of the issuing account. Either the owner or an interested party can send a [NFTokenCreateOffer transaction](../../references/protocol-reference/transactions/transaction-types/nftokencreateoffer.md) to propose buying or selling the `NFToken`; the ledger tracks the proposed transfer as a [NFTokenOffer object](../../references/protocol-reference/ledger-data/ledger-entry-types/nftokenoffer.md), and deletes the `NFTokenOffer` when either side accepts or cancels the offer. If the `NFToken` is transferable, it can be traded multiple times between accounts.

You can destroy a `NFToken` you own using the [NFTokenBurn transaction](../../references/protocol-reference/transactions/transaction-types/nftokenburn.md). If the issuer minted the token with `tfBurnable` flag enabled, the issuer can also burn the token regardless of the current owner. (This could be useful, for example, for a token that represents a ticket to an event which is used up at some point.)

![The NFT Lifecycle](img/nft-lifecycle.png "NFT Lifecycle Image")

For more info about transferring `NFToken` objects, see [Trading NFTokens on the XRP Ledger](non-fungible-token-transfers.md).
