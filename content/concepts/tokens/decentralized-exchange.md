...
The XRP Ledger has possibly the world's oldest _decentralized exchange_ (sometimes abbreviated "DEX"), operating continuously since the XRP Ledger's launch in 2012. The exchange allows users to buy and sell [tokens](tokens.md) for XRP or other tokens, with minimal [fees](../transactions/fees.md) charged to the network itself (not paid out to any party).
...
0. Tran receives the amount of FOO.WayGate that the trade was able to acquire, from the various traders who had previous placed orders to sell it. These tokens go to Tran's [trust line](trust-lines-and-issuing.md) to WayGate for FOO. (If Tran did not already have that trust line, one is automatically created.)
...
- **References:**
    ...
    - [OfferCreate transaction](../../references/protocol-reference/transactions/transaction-types/offercreate.md) to place a new Offer or replace an existing Offer
    - [OfferCancel transaction](../../references/protocol-reference/transactions/transaction-types/offercancel.md) to cancel an existing Offer
    - [Offer object](../../references/protocol-reference/ledger-data/ledger-entry-types/offer.md) for the data structure of passive Offers in the ledger
    - [DirectoryNode object](../../references/protocol-reference/ledger-data/ledger-entry-types/directorynode.md) for the data structure that tracks all the Offers for a given currency pair and exchange rate.
