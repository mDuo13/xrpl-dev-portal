...
Tokens in the XRP Ledger are [fundamentally different than XRP](../../references/protocol-reference/data-types/currency-formats.md#comparison). Tokens always exist _in trust lines_, which _can_ be frozen. XRP exists in accounts, which _cannot_ be frozen.
...
In contrast, when trading in the XRP Ledger's [decentralized exchange](decentralized-exchange.md), you custody your own assets so no one can stop you from dealing in XRP.
