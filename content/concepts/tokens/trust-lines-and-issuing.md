...
Trust lines are structures in the XRP Ledger for holding [tokens](tokens.md). Trust lines enforce the XRP Ledger's rule that you cannot cause someone else to hold a token they don't want. This precaution is necessary to enable the XRP Ledger's use case for [community credit](tokens.md#community-credit) among other benefits.
...
Any account can unilaterally "trust" another account to issue a token by sending a [TrustSet transaction](../../references/protocol-reference/transactions/transaction-types/trustset.md) with a nonzero limit and their own settings. This creates a line with a zero balance, and sets the other side's settings to the default.

Trust lines can be implicitly created by some transactions, such as when you buy a token in the [decentralized exchange](decentralized-exchange.md). In this case, the trust line uses entirely default settings.
...
1. When you acquire more of that token through [trading](decentralized-exchange.md).
...
- The **Limit**, a number from 0 to the [maximum token amount](../../references/protocol-reference/data-types/currency-formats.md). Payments and other accounts' actions cannot cause the trust line's balance (from this account's perspective) to go over the limit. The default is `0`.
...
1. Send a [TrustSet transaction](../../references/protocol-reference/transactions/transaction-types/trustset.md) to set your settings to the defaults.
2. Offload any positive balance you have on the trust line. You could do this by sending a [payment](../payment-types/cross-currency-payments.md), or by selling the currency in the [decentralized exchange](decentralized-exchange.md).
...
- **References:**
    ...
    - [RippleState object](../../references/protocol-reference/ledger-data/ledger-entry-types/ripplestate.md) - The data format for trust lines in the ledger's state data.
    - [TrustSet transaction](../../references/protocol-reference/transactions/transaction-types/trustset.md) - The transaction to create or modify trust lines.
