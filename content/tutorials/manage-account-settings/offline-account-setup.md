...
The offline machine needs secure persistent storage (for example, an encrypted disk drive) and a way to [sign transactions](../../concepts/transactions/set-up-secure-signing.md). For an offline machine, you typically use physical media to transfer any necessary software after downloading it from an online machine. You must be sure that the online machine, the physical media, and the software itself are not infected with malware.

Software options for signing on the XRP Ledger include:

- [Install `rippled`](install-rippled.html) from a package (`.deb` or `.rpm` depending on which Linux distribution you use) file, then [run it in stand-alone mode](../../concepts/networks-and-servers/rippled-server-modes.md).
...
- See also: [Set Up Secure Signing](../../concepts/transactions/set-up-secure-signing.md)
...
- [Require Auth](../../concepts/tokens/authorized-trust-lines.md#enabling-require-auth) so that users can't open [trust lines](../../concepts/tokens/trust-lines-and-issuing.md) to you without your permission. If you don't plan to use the XRP Ledger's decentralized exchange or [token](../../concepts/tokens/tokens.md) features, you may want to do this as a precaution.
...
Retry submitting any transactions that failed with a [non-final outcome](../../concepts/transactions/finality-of-results.md). There is no chance of the same transaction being processed more than once.

### {{n.next()}}. Confirm the final status of the transactions.

For each transaction you submitted, note the transaction's [final outcome](../../concepts/transactions/finality-of-results.md), for example using the [tx method](../../references/http-websocket-apis/public-api-methods/transaction-methods/tx.md). For example:
...
- If the transaction failed with any [`tem`-class code](../../references/protocol-reference/transactions/transaction-results/tem-codes.md), you probably made a typo or another error in constructing the transaction. Double-check the transaction so that you can replace it with a validly-formed one.
- If the transaction failed with a [`tec`-class code](../../references/protocol-reference/transactions/transaction-results/tec-codes.md), you should address it on a case-by-case basis depending on the exact reason it failed.
...
- **References:**
    - [Basic Data Types: Account Sequence](../../references/protocol-reference/data-types/basic-data-types.md#account-sequence)
