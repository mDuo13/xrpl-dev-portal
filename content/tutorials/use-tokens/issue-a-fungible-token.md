...
| [Tick Size][]                | 5                   | Limit the number of decimal places in exchange rates for your token in the [decentralized exchange](../../concepts/tokens/decentralized-exchange.md). A tick size of 5-6 reduces churn of almost-equivalent offers and speeds up price discovery compared to the default of 15. |
...
The following code sample shows how to send an [AccountSet transaction](../../references/protocol-reference/transactions/transaction-types/accountset.md) to enable the recommended cold address settings:
...
The following code sample shows how to send an [AccountSet transaction](../../references/protocol-reference/transactions/transaction-types/accountset.md) to enable the recommended hot address settings:
...
Before you can receive tokens, you need to create a [trust line](../../concepts/tokens/trust-lines-and-issuing.md) to the token issuer. This trust line is specific to the [currency code](../../references/protocol-reference/data-types/currency-formats.md#currency-codes) of the token you want to issue, such as USD or FOO. You can choose any currency code you want; each issuer's tokens are treated as separate in the XRP Ledger protocol. However, users' balances of tokens with the same currency code can [ripple](../../concepts/tokens/rippling.md) between different issuers if the users enable rippling settings.
...
To create a trust line, send a [TrustSet transaction](../../references/protocol-reference/transactions/transaction-types/trustset.md) from the **hot address** with the following fields:
...
The following code sample shows how to send a [TrustSet transaction](../../references/protocol-reference/transactions/transaction-types/trustset.md) from the hot address, trusting the issuing address for a limit of 1 billion FOO:
...
**Note:** If you use [Authorized Trust Lines][], there is an extra step after this one: the cold address must approve the trust line from the hot address. For details of how to do this, see [Authorizing Trust Lines](../../concepts/tokens/authorized-trust-lines.md#authorizing-trust-lines).
...
Now you can create tokens by sending a [Payment transaction](../../references/protocol-reference/transactions/transaction-types/payment.md) from the cold address to the hot address. This transaction should have the following attributes (dot notation indicates nested fields):

| Field | Value |
|---|---|
| `TransactionType` | `"Payment"` |
| `Account` | The cold address issuing the token. |
| `Amount` | An [token amount](../../references/protocol-reference/data-types/basic-data-types.md#specifying-currency-amounts) specifying how much of which token to create. |
...
You can use [auto-filled values](../../references/protocol-reference/transactions/transaction-common-fields.md#auto-fillable-fields) for all other required fields.

The following code sample shows how to send a [Payment transaction](../../references/protocol-reference/transactions/transaction-types/payment.md) to issue 88 FOO from the cold address to the hot address:
...
¹ Users can hold your token without explicitly creating a trust line if they buy your token in the [decentralized exchange](../../concepts/tokens/decentralized-exchange.md). Buying a token in the exchange [automatically creates the necessary trust lines](../../concepts/tokens/offers.md#offers-and-trust). This is only possible if someone is selling your token in the decentralized exchange.
