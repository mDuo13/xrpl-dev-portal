...
- To properly handle XRP values without rounding errors, you need access to a number type that can do math on 64-bit unsigned integers. The examples in this tutorial use [big.js](https://github.com/MikeMcl/big.js/). If you are working with [tokens](../../concepts/tokens/tokens.md), you need even more precision. For more information, see [Currency Precision](../../references/protocol-reference/data-types/currency-formats.md#xrp-precision).
...
**Tip:** By default, connecting to a local `rippled` server gives you access to the full set of [admin methods](../../references/http-websocket-apis/admin-api-methods/admin-api-methods.md) and admin-only data in some responses such as [server_info][server_info method], plus the [public methods](../../references/http-websocket-apis/public-api-methods/public-api-methods.md) that are available when you connect to public servers over the internet.
...
When you subscribe to an account, you get messages for _all transactions to or from the account_, as well as _transactions that affect the account indirectly_, such as trading its [tokens](../../concepts/tokens/tokens.md). If your goal is to recognize when the account has received incoming payments, you must filter the transactions stream and process the payments based on the amount they actually delivered. Look for the following information:

- The **`validated` field** indicates that the transaction's outcome is [final](../../concepts/transactions/finality-of-results.md). This should always be the case when you subscribe to `accounts`, but if you _also_ subscribe to `accounts_proposed` or the `transactions_proposed` stream then the server sends similar messages on the same connection for unconfirmed transactions. As a precaution, it's best to always check the `validated` field.
- The **`meta.TransactionResult` field** is the [transaction result](../../references/protocol-reference/transactions/transaction-results/transaction-results.md). If the result is not `tesSUCCESS`, the transaction failed and cannot have delivered any value.
...
- The **`transaction.TransactionType` field** is the type of transaction. The transaction types that can possibly deliver currency to an account are as follows:
    - **[Payment transactions](../../references/protocol-reference/transactions/transaction-types/payment.md)** can deliver XRP or [tokens](../../concepts/tokens/tokens.md). Filter these by the `transaction.Destination` field, which contains the address of the recipient, and always use the `meta.delivered_amount` to see how much the payment actually delivered. XRP amounts are [formatted as strings](../../references/protocol-reference/data-types/basic-data-types.md#specifying-currency-amounts).
    ...
    - **[CheckCash transactions](../../references/protocol-reference/transactions/transaction-types/checkcash.md)** allow an account to receive money authorized by a different account's [CheckCreate transaction](../../references/protocol-reference/transactions/transaction-types/checkcreate.md). Look at the metadata of a **CheckCash transaction** to see how much currency the account received.

    - **[EscrowFinish transactions](../../references/protocol-reference/transactions/transaction-types/escrowfinish.md)** can deliver XRP by finishing an [Escrow](../../concepts/payment-types/escrow.md) created by a previous [EscrowCreate transaction](../../references/protocol-reference/transactions/transaction-types/escrowcreate.md). Look at the metadata of the **EscrowFinish transaction** to see which account received XRP from the escrow and how much.

    - **[OfferCreate transactions](../../references/protocol-reference/transactions/transaction-types/offercreate.md)** can deliver XRP or tokens by consuming offers your account has previously placed in the XRP Ledger's [decentralized exchange](../../concepts/tokens/decentralized-exchange.md). If you never place offers, you cannot receive money this way. Look at the metadata to see what currency the account received, if any, and how much.

    - **[PaymentChannelClaim transactions](../../references/protocol-reference/transactions/transaction-types/paymentchannelclaim.md)** can deliver XRP from a [payment channel](../../concepts/payment-types/payment-channels.md). Look at the metadata to see which accounts, if any, received XRP from the transaction.

    - **[PaymentChannelFund transactions](../../references/protocol-reference/transactions/transaction-types/paymentchannelfund.md)** can return XRP from a closed (expired) payment channel to the sender.

- The **`meta` field** contains [transaction metadata](../../references/protocol-reference/transactions/transaction-metadata.md), including exactly how much of which currency or currencies was delivered where. See [Look Up transaction Results](../../concepts/transactions/look-up-transaction-results.md) for more information on how to understand transaction metadata.
...
- [Look Up Transaction Results](../../concepts/transactions/look-up-transaction-results.md) to see exactly what a transaction did, and build your software to react appropriately.
...
- **Concepts:**
    ...
    - [Finality of Results](../../concepts/transactions/finality-of-results.md) - How to know when a transaction's success or failure is final. (Short version: if a transaction is in a validated ledger, its outcome and metadata are final.)
- **Tutorials:**
    - [Reliable Transaction Submission](../../concepts/transactions/reliable-transaction-submission.md)
    - [Look Up Transaction Results](../../concepts/transactions/look-up-transaction-results.md)
- **References:**
    - [Transaction Types](../../references/protocol-reference/transactions/transaction-types/transaction-types.md)
